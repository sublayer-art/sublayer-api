...
import com.sublayer.api.domain.dto.SignOrderInfo;
...
public class DappCryptoUtil {
    ...
    public static boolean validate(Integer v, String s, String r, String address, String message) {
        Sign.SignatureData signatureData = new Sign.SignatureData(v.toString().getBytes(), Numeric.hexStringToByteArray(r), Numeric.hexStringToByteArray(s));
        return validate(signatureData, message, address);
    }
    ...
    public static Sign.SignatureData sign(String message, String privateKey){
    ...
    public static VSRSignInfo minerSign(String caddress, String tokenId, String privateKey){
        ...
        Sign.SignatureData signatureData = sign(message, privateKey);
...
//		return order;
        ...
        signStr = Hash.sha3(signStr);
        logger.info("First sha3=>" + signStr);
        ...
        logger.info("Add header=>" + signStr);
        signStr = Numeric.cleanHexPrefix(signStr);
        signStr = Hash.sha3(signStr);
        logger.info("Second sha3=>" + signStr);

        ECKeyPair e = ECKeyPair.create(Numeric.hexStringToByteArray(BUYER_FEE_KEY));
        Sign.SignatureData data = Sign.signMessage(Numeric.hexStringToByteArray(signStr), e, false);
